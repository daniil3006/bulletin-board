...
func (r repository) GetAll(ctx context.Context) ([]ad.Ad, error) {
	q := `
		select id, title, description, price, contact 
		from ads`
	...
	for rows.Next() {
		...
		err = rows.Scan(&ad.ID, &ad.Title, &ad.Description, &ad.Price, &ad.Contact)
		if err != nil {
			return nil, err
		}
...
func (r repository) GetByID(ctx context.Context, ID int) (ad.Ad, error) {
	...
	err := r.client.QueryRow(ctx, q, ID).Scan(&returnedAd.ID, &returnedAd.Title, &returnedAd.Description, &returnedAd.Price, &returnedAd.Contact)
	if err != nil {
		return ad.Ad{}, err
	}
...
func (r repository) Create(ctx context.Context, newAd ad.Ad) (ad.Ad, error) {
	q := `
		insert into ads (title, description, price, contact) 
		values ($1, $2, $3, $4)
		returning id, title, description, price, contact`
	err := r.client.QueryRow(ctx, q, newAd.Title, newAd.Description, newAd.Price, newAd.Contact).
		Scan(&newAd.ID, &newAd.Title, &newAd.Description, &newAd.Price, &newAd.Contact)
	if err != nil {
...
func (r repository) Update(ctx context.Context, newAd ad.Ad, id int) (ad.Ad, error) {
	q := `
		...
		where id = $5
		returning id, title, description, price, contact`
	err := r.client.QueryRow(ctx, q, newAd.Title, newAd.Description, newAd.Price, newAd.Contact, id).
		Scan(&newAd.ID, &newAd.Title, &newAd.Description, &newAd.Price, &newAd.Contact)
	if err != nil {
		return ad.Ad{}, err
	}
